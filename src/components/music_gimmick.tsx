...
export const MusicGimmick: React.FC<Props> = ({ enabled }) => {
  ...
  const [play3, { pause: pause3 }] = useSound(chips3, {
    loop: true,
    volume: 3.5,
  });
  ...
  const play = useCallback(() => {
    if (!enabled) return;
    const array = [play0, play1, play2, play3, play4];
    array[Math.floor(Math.random() * array.length)]();
  }, [play0, play1, play2, play3, play4, enabled]);
  const pause = useCallback(() => {
    ...
    pause3();
    pause4();
  }, [pause0, pause1, pause2, pause3, pause4]);
